...
Codex 插件不再需要 `COMBO_SESSION_COOKIE`。公开入口 `/codex-plugin` 只在 Test runtime 提供，并把当前 `environment`、`sourceSha` 与 `releaseId` 直接写入页面；执行安装前必须用同源 `/version.json` 核对这三项运行身份逐字一致。页面固定显示 `UAT_STATUS=EXTERNAL_EVIDENCE_REQUIRED`，表示 HTTP 页面与部署身份不能替代独立普通用户 UAT 证据，并且不把会随发布变化的部署或 UAT 结论硬编码进运行页面。Project-history 的唯一普通用户 Copy prompt 是单行 `阅读 https://test.43-160-242-46.sslip.io/codex-plugin ，帮我安装 Combo 插件并创建一个新任务。`（UTF-8 111 bytes，SHA-256 `d93995bb094a7d58bd14b34dcc33869627a254694ad51444554441fbfe32525f`），URL 后保留一个 ASCII 空格，避免 Markdown 自动链接吸收中文标点。Plugin 安装在 Codex Host，不是安装到 Project，也不读写 Project 文件；普通用户不打开 Terminal、不输入命令、不提供路径或内部 ID。页面分别定义尚未安装、存在可安全升级的有效旧版和至少为 0.8.6 的当前版三种状态。新 Project-history Agent Package 流程要求 Plugin 已安装启用且是 official stable 有效 semver `>=0.8.6`，MCP name、启用或禁用状态、transport type 与 Test URL 都精确匹配，五个 Project-history 工具已经加载且 readiness 成功。当前版只在 readiness 明确返回 authorization 错误时登录一次，并仅重试该失败的 readiness 一次；当前已加载任务的非 authorization readiness 或内部失败是零 mutation、零 child 的硬阻断。当前 Test UAT 另要求 exact 0.8.6。

只要安装或升级执行过任一 metadata mutation，就永久记录 `metadataMutationAttempted=true`，且该历史优先于冻结任务的表面工具快照。若 official installation metadata 仍未完成，只能使用第二个 typed recovery 摘要；若 official mutation 已完成但 Test MCP entry 暂时 missing 或 unavailable，只能使用第三个摘要；若 post-mutation Plugin 与 Test MCP gate 都精确通过，则无论当前任务表面是否已有五个工具，都必须恰好执行一次 Codex-managed OAuth，登录成功使用第一个 stale-catalog 摘要，登录失败或取消使用第四个 OAuth 摘要。以上 mutation 分支都不能直接创建 business。未发生 mutation 的 current+stale 分支也先登录一次，成功使用第一个摘要，失败或取消使用第四个摘要。只有未发生 mutation 的当前版直通 readiness 成功，或 recovery 任务完成全部校验，才可创建 business。business 任务的唯一首条 prompt 固定为单行 `Combo 插件已经安装好了，请用 Combo 把我以前保存的 Project 里完成过的方法做成一个 Agent：先列出安全的 Project 名称让我选择来源，完整展示将分享的内容，等我确认后创建并核对分享；然后让我选择另一个 Project，并在为它创建的同一个新任务里连续验证两轮。`（UTF-8 345 bytes，SHA-256 `86035706c60165e4e32f01a8b28cd44960f4a607197cb44dc3dae91ebeb8564b`）。projectless setup 任务不得冒充用户选择的第二个 Project 所绑定的验收任务；后者要另行创建，两轮验证必须在同一个后者任务里连续完成。recovery 不得声称已安装、读取 Project 或创建 Agent，修复成功后再另建 business 任务。Host 创建或打开任务不可用时如实阻断，不让用户转用 Terminal。页面保留 legacy current-task Creator 可见入口，命令只放在折叠 fallback 内；Plugin 0.7.0 的既有四工具与旧 Project Agent share 仍可读取。该流程从用户明确选择的 saved Project 做有界 reduced-history 读取，只把 strict Draft candidate 与固定 NOT_PROVEN 来源计数交给 Combo，不使用 Hook、Terminal session 文件、原始 transcript、Project 路径或 ID；render 后经固定确认创建 link-only 可匿名读取、不过期且不可撤回的 immutable Agent Package share，prepare 仍用权威 digest 防混淆。Preview 不提供会错误连接 Test 的跨环境命令，Production 要等独立插件 release 将静态 MCP 地址切到 Production、合并 `main` 并验收后再开放。不要重复 `mcp add`，也不要在聊天中发送 Cookie、验证码或访问令牌。

Recovery 只能使用固定模板 `Combo 安装续接：上一步未完成的低敏摘要是“<summary>”。请只继续核对官方 Combo 0.8.6 安装、Test MCP 与当前任务的 Project-history 能力；不得把本续接任务当作 Project-history 业务已就绪，也不得再创建安装续接任务。修复并验证成功后，创建一个新的 Codex 顶层任务，并把下面这整段话作为唯一第一条消息原样发送：<exact-business-paragraph>`。`<summary>` 按固定顺序只接受 `当前任务在 Combo 0.8.6 安装或升级前启动，尚未加载最新 Skill 和工具目录。`、`Combo 0.8.6 官方安装元数据尚未完成核对。`、`Combo Test MCP 元数据尚未通过核对。`、`Combo OAuth 尚未完成。`，`<exact-business-paragraph>` 只替换为上述 345-byte 固定业务请求；其他摘要、raw 错误、路径、ID、凭据、URL、用户内容或秘密全部拒绝。四个摘要的分类同样固定：第一个只用于 final official metadata 与 Test MCP 精确通过且 mandatory OAuth 成功后的 frozen catalog stale；第二个只用于 initial official absent 或有效旧版在受控 mutation 后仍未完成 official installation metadata；第三个只用于 official mutation 完成后的 Test MCP 暂时 missing 或 unavailable；第四个只用于 OAuth incomplete、登录失败或用户取消。同名错源、orphaned Plugin、无效或不兼容版本是只读、零 mutation、零 child 的硬阻断；MCP name、enabled 或 disabled 状态、transport type、URL 的明确 mismatch 也是零 child 的硬阻断；当前已加载任务的非 authorization readiness 或内部失败不得伪装成 OAuth recovery。初始 bootstrap 的 setup 入口最多创建一个 recovery 任务，其他分类不得创建 recovery；recovery 入口创建 recovery 任务的预算为零，任何失败都不得链式创建。每次允许的 `create_thread` 都只调用一次。只返回 `clientThreadId` 时分类为 `QUEUED`，final 中独立一行仅向 Host 交付 `::created-thread{clientThreadId="..."}` 机器指令，零 wait/read/navigate/重建且不在用户 prose 显示、重复或解释 ID，不声称已打开或业务就绪。只有同一返回同时带非空 `threadId` 和 `hostId` 才是 `READY`；READY 也在 final 中独立一行仅向 Host 交付 `::created-thread{threadId="..."}` 机器指令，用户 prose 不显示、重复或解释 ID，指令不能代替 ready/open 验证。仅 READY 分支恰好调用一次 `wait_threads({targets:[{threadId,hostId}],timeoutMs:0})` 并最多 navigate 一次。create 错误或畸形结果固定报告 `PROJECT_HISTORY_BOOTSTRAP_CREATE_FAILED`；snapshot 或 navigation 失败时用户 prose 只报告 `PROJECT_HISTORY_BOOTSTRAP_OPEN_FAILED`，但必须保留已创建 task 的 threadId 机器指令独立最终行。两者都不重试、不回滚或重建已创建任务，不暴露 raw error、路径、ID、用户内容或秘密。
