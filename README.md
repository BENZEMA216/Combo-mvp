...
DDL 真源在 `db/migrations/`（`0000` 至 `0010`，共 11 个 SQL，字典序即执行序）。`0009` 增加 Agent 使用计费、充值订单和不可变钱包流水；`0010` 增加 Agent Project、不可变 Revision、Runtime Test 与 Release。冻结的 Goal B 部署证据仍停留在 `0008`，在显式开始后续部署目标前不能把它与当前迁移链混作同一份线上证据。
