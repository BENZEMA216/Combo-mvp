...
Runtime 与 authoring 共用 PostgreSQL 和对象存储，但不引用 authoring 源码。它只读 `users`、`auth_sessions`、Capability 定义、不可变 Agent Revision/Release 和 Test Review，读写 `sessions`、`turns`、`messages`、`artifacts`、Agent Test、用量计费表与钱包扣费流水，并且只能更新 `capabilities.ui_artifact_id`。浏览器认证只接受 authoring 签发的不透明会话 Cookie：`SESSION_COOKIE_SECURE=true` 时读取 `__Host-cb_session`，为 false 时读取 `cb_session`。Runtime 只计算 Cookie 摘要并查询 PostgreSQL，不签发会话、不创建用户，也不接受 Bearer 或查询参数令牌。
...
新 MCP 任务可用 `GET /api/v1/runtime/agent-projects/:projectId/tests` 恢复本人 active Project 的最近 Test。查询只接受可选 `limit`（默认 20、最大 50），结果按 `createdAt DESC, id DESC` 排序，包含尚未绑定 Session/Turn 的 `starting`、执行中的 `running` 和终态 Test；列表用于发现 testId/requestKey，既有 `GET /api/v1/runtime/agent-tests/:testId` 继续负责轮询与终态收口，并额外返回 nullable 的完整不可变 Review，使新任务能恢复逐案例 reason、impact、reviewer 与摘要。两个读取面都会显式返回 `qualityStatus` 和 `canPublish`；只有技术 Test 通过、质量复核为 passed 或 accepted_exception、Project 仍 active 且 Revision 仍是当前 Head 时才能发布。缺少复核、复核失败或 Head 已变化都会返回不可发布。他人、不存在或已归档 Project 一律返回 404。
