...
默认关闭的 VNext Consumer surface 使用完全独立的 `CREATOR_AGENT_DATABASE_URL`，readiness 要求 direct-login `combo_agent_consumer_api` 且 `current_user=session_user`、非 superuser、非 `BYPASSRLS`、双向零 role membership、当前数据库只能 `CONNECT` 与使用 PostgreSQL 默认的 session-local `TEMP`（不能 `CREATE` 持久 schema）、不能使用或改写任何 public sequence，权限集合精确。该角色没有 Creator control-plane 写权，也没有任何业务表直接 INSERT/UPDATE；它只能调用两个窄 definer：一个原子写入 `OPENING` Conversation 与 exact `conversation.open` command，另一个在 Runtime 预分配 USER Message UUIDv7 后原子写入 Consumer Message、Invocation、accepted Event、prepare Outbox 与 Conversation projection。ADR-VNEXT-033 将 public `Idempotency-Key` 与 send/retry `clientMessageId` 冻结为 canonical lowercase UUIDv4，服务端 ID 继续使用 UUIDv7。发送消息 HTTP route 与 KMS sealing尚未接线，所有支持部署仍保持feature flag false，且不配置该secret。
...
Turn 创建受数据库部分唯一索引保护。消费请求先按用户与 Capability 预留免费额度或钱包；余额不足在创建 Turn 和用户消息前返回 402。终态状态、错误、消息和计费结算或释放先在 PostgreSQL 事务中共同提交，随后才按 `runId` 幂等写入 Redis 终态。普通事件在写入前确认对应 Turn 仍为 `running`；刷新连接可按 `Last-Event-ID` 补发后继续直播。
