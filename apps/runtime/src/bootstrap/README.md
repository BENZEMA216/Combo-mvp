...
- `routes.ts` 汇总 Capability、Session 和 Artifact 端点，并在共享 API 前缀下注册浏览器事件端点。`ALL_ENDPOINTS` 供测试核对方法、路径和守卫。只有发布身份明确为 Test 时，它才额外注册受 owner、fixture、登录与浏览器来源共同保护的 Combo Miniapp 演示种子端点。
