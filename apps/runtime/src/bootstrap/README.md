...
- `routes.ts` 汇总 Capability、Session、Artifact、待恢复任务和固定 Hosted Test Agent 端点，并在共享 API 前缀下注册浏览器事件端点。固定消费者入口只公开 `combo-knowledge` descriptor 和空 body start；它在创建 Session 前验证 Test gate v2、source SHA 与精确冻结 Package，关闭/漂移返回 404，真实依赖故障返回 503。`ALL_ENDPOINTS` 供测试核对方法、路径和守卫。
