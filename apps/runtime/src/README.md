...
Session 处理器重新校验 owner 并加载 Capability。legacy v1 继续运行 CapabilityDefinition；受控 Test 的 v2 会先按 Registry 和 Package digest 解析固定知识资源，并在 Session 创建时冻结 Release、Package 和资源摘要。发消息请求必须携带 `usageId`。TurnRunner 在事务中锁定 active Session，先判断 owner 免计费、免费额度或钱包余额，再原子插入计费预留、`running` Turn 和轮内用户消息；余额不足时不会创建 Turn，数据库唯一索引保证同一 Session 只有一个运行轮次。
