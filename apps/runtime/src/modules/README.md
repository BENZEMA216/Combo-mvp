...
- `creator-agent-conversation/` 提供默认关闭的邀请制 Creator-hosted Agent local product slice：consumer-only DB authority、Version/Lease/Fence 绑定、`OPENING + conversation.open`、USER send + payload-v2 prepare、transcript/Invocation read 与 terminal-only durable SSE。Test mounted Message/Execution authority失败关闭；cancel/retry、撤权、滑动 TTL 与 Production authority 未落地前，所有受支持部署仍保持 `CREATOR_AGENT_PUBLIC_ENABLED=false`。
