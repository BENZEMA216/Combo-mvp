...
- `creator-agent-conversation/` 提供默认关闭的邀请制 Creator-hosted Agent create groundwork：Version/Lease/Fence 绑定、幂等 Conversation 创建和 VNext HTTP 契约。所有受支持部署都保持 `CREATOR_AGENT_PUBLIC_ENABLED=false`，完整 open/ready、撤权、限额/TTL 与 consumer-only DB authority 落地前不得启用。
