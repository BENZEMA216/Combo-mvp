...
import {
  CREATOR_AGENT_PACKAGE_CAPABILITY_MAX_BYTES,
...
export const LEGACY_CAPABILITY_DOCUMENT_MAX_BYTES = 4 * 1_024 * 1_024;
...
export type LoadedCapabilityDefinition = CapabilityDefinition | CreatorAgentPackageCapability;

export type LoadCapabilityResult =
  | { kind: 'ok'; capability: CapabilitySummary; definition: LoadedCapabilityDefinition }
  | { kind: 'not_found' }
...
export async function readAccessibleCapabilitySummary(
  db: Queryable,
  capabilityId: string,
  userId: string,
  ...
  if (!row || (row.owner_user_id !== userId && (access === 'owner' || !row.published))) {
    return null;
  }
  return {
    id: row.id,
...
export async function loadCapability(
  ...
  let raw: unknown;
  try {
    const bytes = await objectStore.getObjectBounded(
      CAPABILITY_BUCKET,
      row.storage_key,
      LEGACY_CAPABILITY_DOCUMENT_MAX_BYTES,
    );
  ...
  // version 前置判定：不是当前认识的 1/2 → 格式过新（与「结构坏了」区分）。
  const version = (raw as { version?: unknown } | null)?.version;
  if (version !== 1 && version !== 2) return { kind: 'unsupported_version' };
