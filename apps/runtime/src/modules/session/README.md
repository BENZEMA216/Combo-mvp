...
- `handlers.ts` 校验输入、执行 owner 检查；旧会话加载 CapabilityDefinition，Agent Builder 会话加载固定 Revision Bundle，再调用仓储或 TurnRunner。
- `repo.ts` 保存 Session 与 Message 的 SQL、固定 Agent Project/Revision/Release 指针、按 Project 过滤并排除 Test Session、Studio Session 原子复用、归档和锁定逻辑。
