...
- `detail.ts` 在一条 `REPEATABLE READ READ ONLY` 连接内读取 owner、Message、Artifact、当前 UI、active Turn 与权威 receipt。知识会话只投影 user Message 和验证过的 `knowledgeResults`，不从候选 assistant 或 tool transcript 猜测结果。
