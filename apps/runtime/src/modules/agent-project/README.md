...
- `repo.ts` 保存冻结的输出契约、请求摘要、启动租约与 Test 记录。`starting` claim 在任何 Session/Turn 副作用前创建；进程崩溃留下的 claim 过期后可原子接管，而旧租约不能激活 Turn。Project 恢复列表按 `created_at DESC, id DESC` 返回 `starting`、`running` 与终态，并保留 request key 供新 MCP 任务精确关联；单 Test GET 继续负责根据 PostgreSQL Turn 终态做单向收口，并返回 nullable 的完整不可变 Review。两个读取面联查 Authoring 写入的 Test Review 并显式计算质量状态和当前发布资格；Runtime 只读复核，不创建或修改人工结论。文本输出必须非空；structured 输出必须是合法 JSON 且通过该 Revision 冻结的同步 JSON Schema，否则以 `AGENT_OUTPUT_INVALID` 失败。

## 上下游

不可变 Bundle 由 `modules/agent/revision-loader.ts` 从对象存储读取并校验摘要。会话和轮次继续使用既有 session 与 agent 模块；UI 通过 artifact 模块从 Revision 固定对象键复制，不读取 Capability 的可变当前 UI。Test Session 不进入普通会话列表。Authoring Release 只接受本模块已经收口为 passed、存在可发布质量复核且仍匹配当前 Head 的同 Revision Test。
