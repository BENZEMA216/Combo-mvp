...
- `mcp-auth.ts` 只挂在集群内 `/internal/mcp/` 路由，验证 Authoring 经内部 HTTP Header 转发的摘要化 OAuth access token、精确 MCP resource、有效期、读写 scope 和账号状态。它不读取 Cookie，也不会改变公开浏览器路由拒绝 `Authorization` 的规则。

## 上下游

能力、会话和产物路由把守卫放在各自的前置处理链中。会话流路由使用流式请求专用守卫，其余业务路由使用普通守卫。

本目录调用 `platform/infra/auth-session.ts` 读取浏览器会话，调用 `platform/infra/mcp-access-token.ts` 验证集群内 MCP 委托，并使用 `platform/http/_helpers.ts` 返回统一错误信封。鉴权上下文、Cookie 与 MCP resource 常量来自 `@cb/shared`。
