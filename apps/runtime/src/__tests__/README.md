...
- `creator-agent-db-authority.test.ts` 验证 VNext readiness 对 exact current/session role、安全属性和 capability drift fail closed。
...
- `modules/creator-agent-conversation/*.test.ts` 验证默认关闭路由、严格 HTTP 反例、OPENING response、consumer-only create、单 durable open command、exact ready/replay/stale fence 和 Outbox 故障全回滚；真实 PG 门显式开启时缺角色密码直接失败，不可假绿 skip。
