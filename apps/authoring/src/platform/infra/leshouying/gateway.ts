...
interface LeshouyingGatewayConfig {
...
export class LeshouyingPaymentGateway implements PaymentGateway {
  ...
  async #post(path: '/v3/h5pay' | '/v3/qrpay' | '/v3/queryorder', body: SigningParameters) {
    ...
    try {
      ...
      if (!response.ok) throw new PaymentGatewayUncertainError();
...
export function createLeshouyingGateway(env: Env, fetchPort?: FetchPort): PaymentGateway {
  if (!env.LESHOUYING_ENABLED) return new DisabledPaymentGateway();
  return new LeshouyingPaymentGateway(
    {
      environment: env.LESHOUYING_ENVIRONMENT,
      institutionNo: env.LESHOUYING_INSTITUTION_NO,
