...
- `redis.ts` 管理队列 Redis 与热态 Redis。热态实例承载进度流、锁、认证软限流和 Fastify 路由限流，不保存身份或会话真值；两个 API replica 必须连接同一实例。路由限流按发布环境隔离 key，Redis 故障时不退化为副本本地计数。热态命令只允许一次请求重试并使用短连接超时，使 challenge 与受限 HTTP 路由在连接失败时快速失败关闭、verification 快速回落 PostgreSQL 硬限制，同时保留后台重连。
