...
这个目录按业务领域分成五个模块。`account/` 管邮箱验证码、首次建号和 PostgreSQL 会话，`task/` 管任务生命周期、助手上传与提取流水线，`capability/` 管历史能力项读取与发布，`agent-package-release/` 管受控 Test canonical Package Registry，`billing/` 管钱包读取、配置化充值订单、支付确认和主动查单。
