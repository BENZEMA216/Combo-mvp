...
这个目录按业务领域分成六个模块。`account/` 管邮箱验证码、首次建号和 PostgreSQL 会话，`task/` 管任务生命周期、助手上传与提取流水线，`capability/` 管能力项读取与发布，`agent-project/` 管 Agent Project、不可变 Revision、Test Review、编译与 Release，`billing/` 管钱包读取、配置化充值订单、支付确认和主动查单，`external-mcp/` 管 Codex 的 OAuth 与无状态远程 Agent Builder 工具。
