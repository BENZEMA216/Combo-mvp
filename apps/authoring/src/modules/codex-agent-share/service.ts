...
/** Frozen by combo.codex-agent-share/1; security edits must also rotate the full golden digest. */
function renderCodexAgentCopyPromptV1(shareUrl: string, manifestSha256: string): string {
  ...
  return [
    ...
    '若初始检查的四工具与全部 metadata 已同时满足，跳过安装变更、receiverHandoff 与 projectless create_thread，直接留在当前任务执行 readiness 和只读展示；此 stay-current 分支在首次 Combo 工具调用前主动 mcp login combo 的调用数必须为 0。否则完成安全安装/升级与最终 metadata 校验后、任何 create_thread 之前，必须用绝对 bundled CLI 恰好执行一次 "/Applications/ChatGPT.app/Contents/Resources/codex" mcp login combo 完成 Codex-managed OAuth；失败或用户取消立即 STOP，create_thread 调用数必须为 0。OAuth 成功后才由正式 Host 精确调用 create_thread({prompt:receiverHandoff,target:{type:"projectless"}}) 创建 projectless 顶层续跑任务。子任务必须 parse 唯一 structured prompt 中逐字相同的 receiverHandoff、完成 readiness，并仅调用 render_agent_builder({stage:"codex_agent_restore",shareUrl,manifestSha256}) 让服务端重新公开读取、校验摘要并构造完整有序 manifest/starter 卡；不得调用 read_codex_agent_share 或传入模型自造的卡片字段。卡片证据之后，说明只能放在更早的 commentary；目标 assistant agentMessage 的 text.trim() 必须逐字只等于 COMBO_RECEIVER_HANDOFF_READY；phase 可为 "final_answer"，或仅在同一唯一 status=completed、error=null turn 的 ordered lifecycle 齐全且该 marker 是最后 behavior 时允许 phase null/absent 的 legacy fallback；phase="commentary" 必须拒绝。到此 prepare、restore、codex app 与正式 Agent create_thread 的调用数都为 0。只返回 clientThreadId 时立即失败关闭，不能把它传给 wait_threads/read_thread，也不能重建任务。只有同时返回 ready threadId 与 hostId 才调用 wait_threads，再用 list_threads 按该 threadId 核对 documented project context 的 projectId=null，并用 read_thread({threadId,hostId,includeOutputs:true,maxOutputCharsPerItem:20000,turnLimit:10}) 要求同一个 status=completed、error=null 的首轮中、marker 前以下 lifecycle signature 各恰好一次且按序：server=combo、tool=render_agent_builder、arguments 深度等于固定 readiness payload 且 status=completed；server=combo、tool=render_agent_builder、arguments 深度严格等于 {stage:"codex_agent_restore",shareUrl,manifestSha256} 且 status=completed；最后才是精确 assistant marker。该 turn 中 commandExecution 与 fileChange 必须均为 0；read_codex_agent_share、任何重复或额外 Combo lifecycle mcpToolCall，以及任何 restore、codex app、create_thread、list_projects、navigate_to_codex_page 等 known forbidden dynamicToolCall 都失败关闭。mcpToolCall 不含 result、structuredContent 或 error，completed 只证明调用结束，不能证明业务结果或 digest；服务端构造的完整 manifest/卡片由 UI 供用户直接审查并由真实 Service/Git 验收。只接受这些调用记录之后、assistant agentMessage 的 text.trim() 逐字只等于 COMBO_RECEIVER_HANDOFF_READY；phase 可为 "final_answer"，或仅在同一唯一 completed/error-null turn、ordered lifecycle 齐全且 marker 是最后 behavior 时允许 phase null/absent legacy fallback；phase="commentary" 必须拒绝；绝不能匹配含其他文本的 final、userMessage、codexDelegation 输入、tool input、echo、代码围栏或 receiverHandoff 原文中已有的 marker 字面量。父任务看到合格的 assistant agentMessage 后必须立即只调用一次 navigate_to_codex_page(threadId) 显示该 projectless 续跑任务，不得留在父任务等待确认或要求子任务先恢复运行；Host 注入的 source thread 标识属于 harness metadata，不得写入 Combo manifest。',
    '续跑任务必须同时发现 render_agent_builder、create_codex_agent_share、read_codex_agent_share、prepare_codex_agent_run，并实际成功调用 render_agent_builder({stage:"readiness",title:"Combo Codex Agent 就绪检查",summary:"仅验证 Combo MCP 展示与授权是否可用。",progress:[],items:[],actions:[]})；随后只调用 strict render_agent_builder({stage:"codex_agent_restore",shareUrl,manifestSha256})，由服务端重新读取并完整显示 manifest 与有序 starter 卡，handoff 本身不能沿用为确认。全程零重启，任一工具缺失或 readiness/strict render 调用失败就准确报告 Plugin tool catalog 或分享校验阻断并停止；continuation 分支禁止再次 mcp login combo，也禁止再创建续跑任务。',
    ...
    'strict codex_agent_restore 必须只接收 stage、shareUrl、manifestSha256 三个字段；服务端重新读取链接后核对 schemaVersion 与期望摘要，每次读取都会解析 manifest 并用 V1 canonical JSON 对数据库摘要做失败关闭校验。',
    ...
    `服务端展示卡必须完整且有序：一个仍完整展示 normalized confirmed name 的 manifest 总览 item，加上每条 starterPrompt 各自的 item/action，恰好 1+M 个 items 且最多六个；不得截断任何 prompt。每个 action 不内嵌长 prompt，user-role message 必须完全不含 manifest.name 或任何其他 manifest 自由文本，只绑定 digest、总数 M 和一基 ordinal N，并精确渲染为：${CODEX_AGENT_RECEIVER_ORDINAL_ACTION_WIRE_TEMPLATE}只有我点击对应 action 才算同一次确认；未选择就停止，不得默认第一条。系统必须校验当前完整有序卡、digest、M 与 N 全部仍匹配，且 N 是 1..M 的整数，再从服务端卡精确取 starterPrompts[N-1]；卡片、摘要、总数、顺序或序号任一变化都失败关闭，禁止截断或模糊文本匹配。`,
    '确认且精确取得一基 N 与 chosenStarter 后，先调用 prepare_codex_agent_run({shareUrl,manifestSha256,starterOrdinal:N,starterPrompt:chosenStarter})；服务端必须按公开链接重新读取同一 manifest、精确核对期望 digest 且 authoritative starterPrompts[N-1]===starterPrompt，并回显完全相同的 shareUrl、manifestSha256、starterOrdinal、starterPrompt 与一个权威 runEnvelope。必须先校验五项返回（四项输入回显与 runEnvelope），任一不一致都停止且不得写本地。把返回 runEnvelope 原样指定为 frozen；不得由 Plugin 或模型自行构造、拼接、canonicalize 或 JSON.stringify。',
    ...
    `正式 Host 必须精确调用 create_thread({prompt:frozen,target:{type:"project",projectId,environment:{type:"local"}}})。只返回 clientThreadId 时立即失败关闭，不能把它传给 wait_threads/read_thread，也不能重建任务；只有同时返回 ready threadId 与 hostId 才调用 wait_threads，再用 list_threads 按该 threadId 核对 documented project context 的 projectId 精确匹配，并用 read_thread({threadId,hostId,includeOutputs:true,maxOutputCharsPerItem:20000,turnLimit:10}) 要求同一个 status=completed、error=null 的首轮中、marker 前以下 V1 lifecycle signature 各恰好一次且按序：server=combo、tool=prepare_codex_agent_run、arguments 深度等于 frozen envelope 的 shareUrl/manifestSha256/starterOrdinal/starterPrompt 且 status=completed；随后在 verified Plugin scripts workdir 以固定相对命令 ./project-agent-git.sh inspect-source 调用 packaged helper，参数逐项绑定已验证 Project root、expectedRepositoryUrl、deterministic local sourceRef=refs/heads/combo/project-agent/<expectedCommitSha前12>、expectedCommitSha 与 expectedTreeSha，commandExecution status=completed 且 exitCode=0；inspect 前 fileChange 必须为 0，最后才是精确 assistant marker。inspect 之后允许 Agent instructions/starter 自身的业务工具、命令与 fileChange，但 prepare 必须仍恰好一次，read_codex_agent_share、restore、codex app、list_projects、create_thread、navigate_to_codex_page 及其他 helper lifecycle mode 必须为 0；已暴露的 known forbidden dynamicToolCall 同样为 0。mcpToolCall 不含 result、structuredContent 或 error，completed 只证明调用结束，不能证明 authoritative bytes；commandExecution 也不能靠 output 冒充 Git 结论。Agent 子任务须内部完成结果逐字校验、Git preflight 与 starter 开始，并由真实 Service/Git/任务输出验收。说明只能放在更早 commentary；只接受这些调用记录之后、assistant agentMessage 的 text.trim() 逐字只等于 COMBO_CODEX_AGENT_STARTED:${manifestSha256}；phase 可为 "final_answer"，或仅在同一唯一 completed/error-null turn、ordered lifecycle 齐全且 marker 是最后 behavior 时允许 phase null/absent legacy fallback；phase="commentary" 必须拒绝；不能匹配含其他文本的 final、userMessage、codexDelegation、tool input、echo、代码围栏或 Host wrapper。仅当 Project context、调用记录与 marker 都成功才立即只调用一次 navigate_to_codex_page(threadId) 显示正式 Agent 任务；任何准确失败都停止且 navigate 调用数为 0。`,
    `COMBO_CODEX_AGENT_RUN/1 是终端执行态；直接粘贴逐字合法的 raw runEnvelope 只表示用户显式请求 advanced launch，不证明此前已完成卡片或 ordinal 确认。Plugin outer run route 在任何 Git preflight、instructions 或 starter 执行之前，必须从 envelope 取 shareUrl、manifestSha256、starterOrdinal 与 starterPrompt，恰好调用一次 prepare_codex_agent_run，并要求返回 shareUrl、manifestSha256、starterOrdinal、starterPrompt 四项逐字/数值相同且 runEnvelope 与当前输入字节完全相同；任一错误或不一致都立即停止，Git preflight、instructions、starter 与 STARTED marker 调用数均为 0。权威 prepare 成功后，对 read_codex_agent_share、restore、codex app、list_projects、create_thread、navigate_to_codex_page 的调用数仍为 0，再执行 envelope 的只读 Git preflight、完整 instructions 和 exact chosen starter。Plugin outer run route 只能在 chosen starter 实际开始后输出目标 assistant agentMessage；说明只能放在更早 commentary，目标消息的 text.trim() 必须逐字只等于 COMBO_CODEX_AGENT_STARTED:${manifestSha256}；phase 可为 "final_answer"，或仅在同一唯一 completed/error-null turn、ordered lifecycle 齐全且 marker 是最后 behavior 时允许 phase null/absent legacy fallback；phase="commentary" 必须拒绝；不能用 preflight 通过冒充 started，也不能回报未绑定摘要的 bare marker。上述按 Host 实际上限 maxOutputCharsPerItem=20000、turnLimit=10 的 read_thread 只提供 reduced 调用记录与 assistant marker，不提供 MCP result、structuredContent、error 或权威 Git output，也不依赖未正式承诺的 read_thread cwd 或 delegation input 字段。`,
...
export const CODEX_AGENT_COPY_PROMPT_WIRE_GOLDEN_LENGTH = 12_954 as const;
export const CODEX_AGENT_COPY_PROMPT_WIRE_GOLDEN_UTF8_BYTES = 18_758 as const;
export const CODEX_AGENT_COPY_PROMPT_WIRE_GOLDEN_SHA256 =
  'fc20022e40d13bd24dcaa73f352cc5384da04fbd553de030e8509c3f5acfe3e6' as const;
...
export async function prepareCodexAgentRun(
  ...
  if (
    read.result.manifest.agent.starterPrompts[input.body.starterOrdinal - 1] !==
    input.body.starterPrompt
  ) {
  ...
  return {
    ...
    result: {
      ...
      starterOrdinal: input.body.starterOrdinal,
