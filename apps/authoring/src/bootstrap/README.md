...
- `routes.ts` 把 account、task、capability、agent-project、project-agent-share、billing 与浏览器观测路由统一挂到 `/api/v1`，并导出完整端点声明供测试核对；`external-mcp/routes.ts` 由 `app.ts` 直接挂根级规范路径。

## 上下游

`processes/api.ts` 调用 `buildApp` 后监听端口。`app.ts` 依赖 `platform/config/env.ts`、`platform/infra/index.ts`、`platform/http/`、`platform/observability/node.ts` 和 billing 查单调度器；`routes.ts` 依赖六个浏览器业务模块的路由声明。
