...
- `routes.ts` 把 account、task、capability 与浏览器观测路由统一挂到 `/api/v1`；仅当 `COMBO_ENVIRONMENT=test` 时额外注册受同源与登录守卫保护的 demo seed，并导出按环境展开的端点声明供测试核对。

## 上下游

`processes/api.ts` 调用 `buildApp` 后监听端口。`app.ts` 依赖 `platform/config/env.ts`、`platform/infra/index.ts`、`platform/http/` 和 `platform/observability/node.ts`；`routes.ts` 依赖四个业务模块的路由声明，其中 demo 模块不能出现在非 Test 发布面。
