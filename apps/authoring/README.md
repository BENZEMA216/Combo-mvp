...
本包是创作者业务的唯一写入服务。API 进程提供第一方邮箱验证码认证、任务、上传、提取、能力管理和 Agent Project/Revision/Release 接口；worker 进程消费提取队列并执行租约对账。只有 API 进程持有邮件供应商与验证码 HMAC 密钥，worker 不依赖认证投递配置。
