...
import type { AgentExecutionDependencies } from './ports.js';
...
export async function executeCreatorAgentLocalTurn(
  ...
  assertSupportedRuntime(version, dependencies.supportedCodexVersion);
  let snapshot: string;
  ...
  try {
    const result = await dependencies.runInvocation(
      {
...
export function compileCreatorAgentDeveloperInstructionsForRuntime(
  input: unknown,
  ...
  assertSupportedRuntime(version, supportedCodexVersion);
...
export function assertCreatorAgentVersionRunnableForRuntime(
  projectPath: string,
  input: unknown,
  supportedCodexVersion: string,
): void {
...
function assertSupportedRuntime(version: CreatorAgentVersion, supportedCodexVersion: string): void {
  ...
  if (
    ...
    (requirements.codexVersion !== undefined && requirements.codexVersion !== supportedCodexVersion)
  ) {
