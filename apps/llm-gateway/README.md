...
- `src/payment-admission.ts` 同时向 Billing 记录执行尝试结果。重试仍使用原业务编号，计量使用中台返回的本次内部执行编号。
...
模型明确拒绝请求或返回不可用结果时，网关先完成零元结算，再记录明确失败。只有 Billing 确认记录成功，502 响应才带 `x-combo-call-outcome: failed_no_charge`；SDK 据此允许业务用原编号重试。下一次执行由中台串行分配，最多二十次，不重新创建支付单。普通 502、网络断开和结果记录不确定都不是安全重试证明。
