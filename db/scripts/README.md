...
- `provision-app-roles.ts` 在迁移已收口授权后，通过绑定参数为 legacy 三角色、VNext control-plane 三角色与独立 Consumer-only 角色分组启用登录。它不输出密码或拼入迁移文件；`POSTGRES_AGENT_CONSUMER_API_PASSWORD` 不要求与尚未启用公网面的部署一同配置。
