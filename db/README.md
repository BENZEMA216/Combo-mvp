...
这个目录是数据库结构的唯一真源。当前迁移链从 `0000` 连续到 `0013`，已经执行的迁移保持原样；第一方邮箱认证、应用数据库角色、共享 Agent 计费与 Creator-hosted Agent VNext 只通过后续迁移追加。
...
MIGRATION_RUNS=2 EXPECTED_MIGRATION_HEAD=0013_creator_agent_consumer_create.sql pnpm -F @cb/db migrate
