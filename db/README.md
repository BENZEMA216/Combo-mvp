...
这个目录是数据库结构的唯一真源。当前迁移链从 `0000` 连续到 `0013`，共 14 个 SQL；已经发布的 `0000` 至 `0012` 保持原样，远程 MCP 认证只通过后续迁移追加。所有环境都从空业务数据建立当前结构，不提供旧身份或旧 Preview 数据桥接。
...
MIGRATION_RUNS=2 EXPECTED_MIGRATION_HEAD=0013_external_mcp_oauth.sql pnpm -F @cb/db migrate
