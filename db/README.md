...
这个目录是数据库结构的唯一真源。当前迁移链从 `0000` 连续到 `0012`，共 13 个 SQL；已经发布的 `0000` 至 `0011` 保持原样，Agent Builder 聚合层只通过后续迁移追加。所有环境都从空业务数据建立当前结构，不提供旧身份或旧 Preview 数据桥接。
...
MIGRATION_RUNS=2 EXPECTED_MIGRATION_HEAD=0012_agent_builder_v1.sql pnpm -F @cb/db migrate
