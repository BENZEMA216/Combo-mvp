...
这个目录是数据库结构的唯一真源。当前迁移链从 `0000` 连续到 `0008`，已经发布的 `0000` 至 `0006` 保持原样；第一方邮箱认证和应用数据库角色只通过后续迁移追加。所有环境都从空业务数据建立当前结构，不提供旧身份或旧 Preview 数据桥接。
...
`users` 是业务主体真源。`tasks` 与 `uploads` 保存创作流水线状态。`capabilities` 保存能力索引、定义对象键和当前 UI 指针。`sessions`、`turns`、`messages` 与 `artifacts` 保存试用和 Studio 状态；大内容仍在 MinIO。认证表只保存规范身份以及验证码、目标和 Cookie 的摘要，不保存验证码、Cookie 或供应商令牌原文。
...
Authoring API 可以读写认证表、任务、上传、能力和模型审计。Worker 只能处理任务、上传、能力和模型审计，不能读取或写入认证表。Runtime 只读 `users` 与 `auth_sessions`，读写 Session、Turn、Message 与 Artifact；它只获得 `capabilities.ui_artifact_id` 的列级更新权限，用于成功 Turn 的 current UI 晋升，不能修改 Capability 其他字段。
...
MIGRATION_RUNS=2 EXPECTED_MIGRATION_HEAD=0008_application_database_roles.sql pnpm -F @cb/db migrate
pnpm -F @cb/db migrate:status
node --experimental-strip-types db/scripts/migrate.ts --head
pnpm -F @cb/db test
```

`MIGRATION_RUNS=2` 会在同一连接与 advisory lock 内重新读取并严格验证完整账本。真实 PostgreSQL 集成还必须证明空库执行、第二次幂等、`0007` 非空用户门禁和三个应用角色的正负权限。
