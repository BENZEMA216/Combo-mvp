...
- `v2-migrate-runner.test.ts` 核对 V2 runner 只复用 canonical `0000` 至 `0011`，随后执行独立 `0012` 至 `0014`，并兼容已部署的 V2 账本。
- `provision-v2-app-roles.test.ts` 核对 V2 的五角色密码必须完整成组提供，且密码只作为绑定值进入 PostgreSQL。
- `application-database-v2-roles.pg.test.ts` 在显式启用的独立 V2 PostgreSQL 数据库中，以五个真实角色登录并验证 V2 身份、钱包、流水与计量的最小权限。
