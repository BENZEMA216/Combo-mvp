...
- `creator-agent-consumer-message-accept.pg.test.ts` 以 direct-login `combo_agent_consumer_api` 在真实 PostgreSQL 证明 full-accept 原子链、`combo_runtime` 预分配 Message ID、数据库 ID/时钟、并发 replay/conflict、context-limit 单调 marker、Version/Lease fresh fence、跨 Consumer/旧 tuple 拒绝、最小 ACL 和 late-Outbox failure 整体回滚；只有显式 E2 开关与专用角色密码同时存在时运行。
