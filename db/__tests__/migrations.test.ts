...
describe('migrations', () => {
  ...
  it('keeps authentication, roles, billing, and Agent Builder after Goal B schema migrations', () => {
    ...
    expect(list.slice(-4)).toEqual([
      ...
      '0009_billing.sql',
      '0010_agent_builder_v1.sql',
    ]);
  });

  it('0010 freezes Revision and Release while pinning Test and Session to exact hashes', () => {
    const sql = readFileSync(join(MIGRATIONS_DIR, '0010_agent_builder_v1.sql'), 'utf-8');
