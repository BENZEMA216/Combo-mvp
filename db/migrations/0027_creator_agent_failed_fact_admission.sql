-- 0027 · Database-owned confirmed invocation.failed terminal admission.
--
-- One Broker-only authority owns the failed projection, Worker Event, Consumer terminal payload,
...
DO $failed_fact_zero_legacy$
BEGIN
  IF EXISTS (
    SELECT 1 FROM public.agent_invocation_events AS event
     WHERE event.source = 'WORKER' AND event.event_type = 'invocation.failed'
  ) OR EXISTS (
    SELECT 1 FROM public.agent_invocations AS invocation
     WHERE invocation.state = 'FAILED'
       AND invocation.error_code IN (
    ...
    RAISE EXCEPTION '0027 requires zero legacy confirmed Worker failed terminals'
      USING ERRCODE = '55000';
  END IF;
END;
$failed_fact_zero_legacy$;

CREATE TABLE public.creator_agent_failed_terminal_receipts (
  ...
  CONSTRAINT fk_failed_terminal_receipt_invocation_tenant
  ...
  CONSTRAINT fk_failed_terminal_receipt_event
    FOREIGN KEY (terminal_event_id, invocation_id)
    REFERENCES public.agent_invocation_events (id, invocation_id)
);

CREATE TRIGGER creator_agent_failed_terminal_receipts_immutable
BEFORE UPDATE OR DELETE ON public.creator_agent_failed_terminal_receipts
FOR EACH ROW EXECUTE FUNCTION public.reject_creator_agent_immutable_mutation();

CREATE TRIGGER creator_agent_failed_terminal_receipts_no_truncate
BEFORE TRUNCATE ON public.creator_agent_failed_terminal_receipts
FOR EACH STATEMENT EXECUTE FUNCTION public.reject_creator_agent_immutable_mutation();

ALTER TABLE public.creator_agent_failed_terminal_receipts ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.creator_agent_failed_terminal_receipts FORCE ROW LEVEL SECURITY;

REVOKE ALL PRIVILEGES ON public.creator_agent_failed_terminal_receipts FROM PUBLIC;
REVOKE ALL PRIVILEGES ON public.creator_agent_failed_terminal_receipts FROM
  combo_agent_api,
  combo_agent_broker,
...
CREATE OR REPLACE FUNCTION public.creator_agent_worker_failed_fact_digest_v1(
  ...
  SELECT pg_catalog.encode(
    public.digest(
      pg_catalog.convert_to(
        ...
        ',"errorCode":' || pg_catalog.to_jsonb(input_error_code)::text ||
...
CREATE OR REPLACE FUNCTION public.creator_agent_failed_consumer_payload_digest_v1(
...
CREATE OR REPLACE FUNCTION public.creator_agent_failed_consumer_dedupe_key_v1(
  input_owner_id uuid,
...
REVOKE ALL ON FUNCTION public.creator_agent_worker_failed_fact_digest_v1(
  uuid, uuid, text, text, text, uuid, bigint, text
) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.creator_agent_failed_consumer_payload_digest_v1(
  uuid, uuid, text, text
) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.creator_agent_failed_consumer_dedupe_key_v1(uuid, bigint)
  FROM PUBLIC;

CREATE OR REPLACE FUNCTION public.creator_agent_project_failed_fact_v1(
...
BEGIN
  ...
  IF session_user <> 'combo_agent_broker'
    ...
    RAISE EXCEPTION 'Failed fact admission requires exact Broker session authority'
      USING ERRCODE = '42501';
  END IF;
  IF input_creator_id IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'Failed fact admission requires exact Creator and cleared Consumer context'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF input_creator_id IS NULL OR input_installation_id IS NULL
     ...
     OR input_error_code IS NULL OR input_fact_digest IS NULL
  ...
  recomputed_fact_digest := public.creator_agent_worker_failed_fact_digest_v1(
  ...
  LOOP
    SELECT event.id, event.invocation_id, event.creator_id, event.consumer_subject_id,
      ...
      LEFT JOIN public.creator_agent_failed_terminal_receipts AS receipt
    ...
    IF FOUND THEN
      IF existing.event_type = 'invocation.failed' THEN
        ...
        existing_recomputed_digest := public.creator_agent_worker_failed_fact_digest_v1(
        ...
        payload_digest_value := public.creator_agent_failed_consumer_payload_digest_v1(
          existing.conversation_id, existing.invocation_id,
          existing.error_code, terminal_occurred_at_text
        );
        dedupe_key_value := public.creator_agent_failed_consumer_dedupe_key_v1(
          existing.consumer_subject_id, existing.id
        );
        IF existing.source_fact_digest IS DISTINCT FROM existing_recomputed_digest
        ...
        terminal_payload := pg_catalog.jsonb_build_object(
          ...
          'errorCode', existing.error_code, 'occurredAt', terminal_occurred_at_text
        );
        ...
        existing_identity := pg_catalog.jsonb_build_object(
          ...
          'eventType', 'invocation.failed', 'payload', existing.payload,
        ...
        IF existing_identity = received_identity THEN
          RETURN QUERY
            SELECT 'EXACT'::text, existing.error_code, existing.terminal_at,
                   consumer_cursor_value, false, false, false, false, false, false,
      ...
      ELSIF existing.event_type = 'invocation.succeeded'
        ...
        received_identity := pg_catalog.jsonb_build_object(
          ...
          'invocationId', input_invocation_id::text,
          'source', 'WORKER', 'sourceEventId', input_source_event_id::text,
          'eventType', 'invocation.failed', 'fact', received_fact,
    ...
    BEGIN
      ...
      UPDATE public.agent_invocations AS invocation
         SET state = 'FAILED', result_message_id = NULL, result_digest = NULL,
             error_code = input_error_code, uncertainty_reason = NULL,
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'failed projection changed unexpectedly' USING ERRCODE = '55000';
      END IF;
      ...
      SELECT input_invocation_id, input_creator_id, incoming.consumer_subject_id,
             ...
             input_source_event_id::text, 'invocation.failed',
             pg_catalog.jsonb_build_object('state', 'FAILED', 'errorCode', input_error_code),
      ...
      terminal_payload := pg_catalog.jsonb_build_object(
        ...
        'invocationId', input_invocation_id::text, 'terminalState', 'FAILED',
        'assistantMessageId', NULL::text, 'resultDigest', NULL::text,
        'errorCode', input_error_code, 'occurredAt', terminal_occurred_at_text
      );
      payload_digest_value := public.creator_agent_failed_consumer_payload_digest_v1(
        incoming.conversation_id, input_invocation_id,
        input_error_code, terminal_occurred_at_text
      );
      dedupe_key_value := public.creator_agent_failed_consumer_dedupe_key_v1(
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'failed Conversation did not return to IDLE' USING ERRCODE = '55000';
      END IF;
    ...
    RETURN QUERY
      SELECT 'ADMITTED'::text, input_error_code, terminal_at_value,
...
REVOKE ALL ON FUNCTION public.creator_agent_project_failed_fact_v1(
  uuid, uuid, text, integer, text, uuid, uuid, text, text, text,
  uuid, bigint, text, text
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.creator_agent_project_failed_fact_v1(
  uuid, uuid, text, integer, text, uuid, uuid, text, text, text,
  uuid, bigint, text, text
...
BEGIN
  ...
  IF trusted_owner IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'Failed fact admission requires a trusted SECURITY DEFINER owner'
      USING ERRCODE = '42501';
  END IF;
END;
...
CREATE OR REPLACE FUNCTION public.enforce_creator_agent_failed_consumer_outbox_insert()
RETURNS trigger AS $failed_outbox_authority$
DECLARE
  privileged_session boolean;
  admission_owner name;
  is_failed_terminal boolean;
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM public.agent_invocation_events AS event
     WHERE event.id = NEW.source_event_id AND event.invocation_id = NEW.invocation_id
       AND event.source = 'WORKER' AND event.event_type = 'invocation.failed'
  ) INTO is_failed_terminal;
  IF NOT is_failed_terminal THEN RETURN NEW; END IF;
  ...
  IF session_user <> 'combo_agent_broker' OR current_user = session_user
    ...
    RAISE EXCEPTION 'failed Consumer terminal requires failed fact admission authority'
      USING ERRCODE = '42501';
  END IF;
  RETURN NEW;
END;
...
REVOKE ALL ON FUNCTION public.enforce_creator_agent_failed_consumer_outbox_insert() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.enforce_creator_agent_failed_consumer_outbox_insert()
  TO combo_agent_broker, combo_agent_reconciler;

CREATE TRIGGER consumer_event_outbox_failed_insert_authority
BEFORE INSERT ON public.consumer_event_outbox
FOR EACH ROW EXECUTE FUNCTION public.enforce_creator_agent_failed_consumer_outbox_insert();

CREATE OR REPLACE FUNCTION public.enforce_creator_agent_confirmed_failed_companion()
RETURNS trigger AS $failed_companion$
DECLARE exact_chain boolean;
BEGIN
  IF NEW.state <> 'FAILED' OR NEW.error_code NOT IN (
    'SNAPSHOT_DIGEST_MISMATCH', 'PROTOCOL_INCOMPATIBLE',
    'SANDBOX_ATTESTATION_FAILED', 'RUNTIME_START_FAILED',
    'MODEL_QUOTA_EXHAUSTED', 'TURN_TIMEOUT', 'TURN_FAILED'
  ) THEN
  ...
  SELECT EXISTS (
    SELECT 1
      FROM public.agent_invocation_events AS event
      JOIN public.creator_agent_failed_terminal_receipts AS receipt
     ...
     WHERE event.invocation_id = NEW.id
       ...
       AND event.payload = pg_catalog.jsonb_build_object(
         'state', 'FAILED', 'errorCode', NEW.error_code
       )
  ...
  IF exact_chain IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'confirmed FAILED projection requires exact terminal chain'
      USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
...
REVOKE ALL ON FUNCTION public.enforce_creator_agent_confirmed_failed_companion() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.enforce_creator_agent_confirmed_failed_companion()
  TO combo_agent_broker, combo_agent_reconciler;

CREATE CONSTRAINT TRIGGER agent_invocations_confirmed_failed_companion
AFTER UPDATE OF state, error_code, terminal_at ON public.agent_invocations
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION public.enforce_creator_agent_confirmed_failed_companion();
...
BEGIN
  SELECT procedure.prosecdef AND (role.rolsuper OR role.rolbypassrls)
    ...
    JOIN pg_catalog.pg_roles AS role ON role.oid = procedure.proowner
   WHERE procedure.oid =
     'public.enforce_creator_agent_confirmed_failed_companion()'::regprocedure;
  IF trusted_owner IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'Failed companion requires a trusted SECURITY DEFINER owner'
      USING ERRCODE = '42501';
  END IF;
END;
