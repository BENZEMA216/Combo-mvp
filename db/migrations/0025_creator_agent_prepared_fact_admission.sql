-- 0025 · Database-owned invocation.prepared Worker fact admission.
--
-- The authority owns only the prepared fact's three atomic mutations: Invocation -> PERSISTED,
-- the append-only Worker Event, and SENT prepare-command acknowledgement. Start-command creation
-- and late START_DISPATCH_UNKNOWN reconciliation remain in the Cloud projector's outer transaction.
...
-- Exact RFC 8785 bytes for combo.worker-invocation-fact/1 invocation.prepared. Every dynamic
-- value is serialized as a JSON string and the ASCII keys are frozen in JCS order.
  ...
  SELECT pg_catalog.encode(
    public.digest(
      pg_catalog.convert_to(
        ...
        ',"prepareCommandId":' || pg_catalog.to_jsonb(input_prepare_command_id::text)::text ||
        ',"protocol":"combo.worker-invocation-fact/1"' ||
        ',"requestDigest":' || pg_catalog.to_jsonb(input_request_digest)::text ||
        ',"schemaVersion":1' ||
...
REVOKE ALL ON FUNCTION public.creator_agent_worker_prepared_fact_digest_v1(
  uuid, uuid, text, text, text, uuid, bigint, text, uuid
) FROM PUBLIC;

-- Existing prepared facts must be reconstructible from immutable Cloud authority before the old
-- Broker writer is cut off. Writers are already locked, so this is a stable fail-closed gate.
DO $prepared_fact_zero_legacy$
BEGIN
  IF EXISTS (
    SELECT 1
      ...
      LEFT JOIN public.broker_outbox AS command
        ON command.command_id = event.broker_command_id
       AND command.creator_id = event.creator_id
       AND command.invocation_id = event.invocation_id
       AND command.consumer_subject_id = event.consumer_subject_id
     WHERE event.source = 'WORKER'
       AND event.event_type = 'invocation.persisted'
       AND (
         event.source_event_id !~
           '^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
         OR event.payload IS DISTINCT FROM '{"state":"PERSISTED"}'::jsonb
         OR event.source_fact_digest IS NULL
         OR event.broker_command_id IS NULL
         OR event.source_event_id IS DISTINCT FROM event.broker_command_id::text
         ...
         OR command.command_type IS DISTINCT FROM 'invocation.prepare'
         OR command.target_worker_id IS DISTINCT FROM invocation.assigned_worker_id
         OR command.conversation_id IS DISTINCT FROM invocation.conversation_id
         OR command.deployment_id IS DISTINCT FROM conversation.deployment_id
         OR command.assignment_lease_id IS DISTINCT FROM invocation.assignment_lease_id
         OR command.assignment_fence IS DISTINCT FROM invocation.assignment_fence
         OR command.execution_capability_id IS DISTINCT FROM invocation.execution_capability_id
         OR command.execution_capability_digest IS DISTINCT FROM
              invocation.execution_capability_digest
         OR command.predecessor_command_id IS NOT NULL
         OR command.state NOT IN ('ACKED', 'EXPIRED')
         OR command.attempt_count < 1
         OR event.source_fact_digest IS DISTINCT FROM
              public.creator_agent_worker_prepared_fact_digest_v1(
                event.source_event_id::uuid,
                event.invocation_id,
    ...
    RAISE EXCEPTION '0025 requires every legacy prepared fact to have exact reconstructible authority'
      USING ERRCODE = '55000';
  END IF;
END;
$prepared_fact_zero_legacy$;
...
BEGIN
  -- Authentication and Creator context precede validation, locks, tenant derivation, or reads.
  ...
  IF session_user <> 'combo_agent_broker'
    ...
    RAISE EXCEPTION 'Prepared fact admission requires exact Broker session authority'
      USING ERRCODE = '42501';
  END IF;
  IF input_creator_id IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'Prepared fact admission requires exact Creator and cleared Consumer context'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF input_creator_id IS NULL
     ...
     OR input_prepare_command_id IS NULL
     ...
     OR input_type IS DISTINCT FROM 'invocation.prepared'
     OR input_source_event_id IS DISTINCT FROM input_prepare_command_id
     ...
     OR input_lease_id::text !~
       '^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
     OR input_fence IS NULL OR input_fence < 1 OR input_fence > 9223372036854775807
    ...
    RETURN QUERY SELECT 'AUTHORITY_REJECTED'::text, NULL::uuid, NULL::boolean;
    RETURN;
  END IF;

  recomputed_fact_digest := public.creator_agent_worker_prepared_fact_digest_v1(
  ...
  PERFORM pg_catalog.set_config(
    'app.consumer_id',
    incoming.consumer_subject_id::text,
    true
  );
  ...
  received_fact := pg_catalog.jsonb_build_object(
    ...
    'requestDigest', input_request_digest,
    'prepareCommandId', input_prepare_command_id::text
  );
  ...
  LOOP
    ...
    IF FOUND THEN
      IF existing.event_type IS DISTINCT FROM 'invocation.persisted'
         OR existing.source IS DISTINCT FROM 'WORKER'
         OR existing.payload IS DISTINCT FROM '{"state":"PERSISTED"}'::jsonb
         OR existing.source_event_id !~
           '^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
         OR existing.source_fact_digest IS NULL
         OR existing.broker_command_id IS NULL
         ...
         OR existing.deployment_id IS NULL
         ...
         OR existing.command_type IS DISTINCT FROM 'invocation.prepare'
         OR existing.command_state NOT IN ('ACKED', 'EXPIRED')
         OR existing.command_attempt_count IS NULL
         OR existing.command_attempt_count < 1
         ...
         OR existing.predecessor_command_id IS NOT NULL
         OR existing.command_capability_id IS DISTINCT FROM existing.execution_capability_id
         OR existing.command_capability_digest IS DISTINCT FROM
              existing.execution_capability_digest
         OR existing.invocation_state NOT IN (
           'PERSISTED', 'STARTING', 'RUNNING', 'CANCEL_REQUESTED', 'RECONCILING',
           'SUCCEEDED', 'FAILED', 'CANCELLED', 'UNCERTAIN'
         ) THEN
        RETURN QUERY SELECT 'INVARIANT_FAILED'::text, NULL::uuid, NULL::boolean;
        RETURN;
      END IF;

      existing_recomputed_digest := public.creator_agent_worker_prepared_fact_digest_v1(
      ...
      existing_fact := pg_catalog.jsonb_build_object(
        ...
        'prepareCommandId', existing.broker_command_id::text
      );
      existing_identity := pg_catalog.jsonb_build_object(
        'domain', 'combo:vnext:worker-prepared-event-identity:v1',
        'protocol', 'combo.worker-invocation-fact/1',
        'version', 1,
        ...
        'fact', existing_fact,
        'factDigest', existing.source_fact_digest
      );
      received_identity := pg_catalog.jsonb_build_object(
        ...
        'sourceEventId', input_source_event_id::text,
        'eventType', 'invocation.persisted',
        'payload', '{"state":"PERSISTED"}'::jsonb,
        'brokerCommandId', input_prepare_command_id::text,
        'fact', received_fact,
        'factDigest', input_fact_digest
      );

      IF existing_identity = received_identity THEN
        RETURN QUERY SELECT 'EXACT'::text, NULL::uuid, NULL::boolean;
        RETURN;
      END IF;
      ...
      IF durable_alert_id IS NULL THEN
        SELECT alert.id
         ...
         WHERE alert.invocation_id = input_invocation_id
           AND alert.reason = 'SOURCE_EVENT_CONFLICT'
           AND alert.source = 'WORKER'
           ...
           AND alert.expected_journal_seq IS NULL
           AND alert.received_journal_seq IS NULL;
        durable_alert_replayed := true;
      ELSE
        durable_alert_replayed := false;
      END IF;
      IF durable_alert_id IS NULL THEN
        RAISE EXCEPTION 'Prepared fact alert dedupe invariant failed'
          USING ERRCODE = '55000';
      END IF;
      RETURN QUERY
        SELECT 'SECURITY_BLOCKED'::text, durable_alert_id, durable_alert_replayed;
      RETURN;
    END IF;
    ...
    IF incoming.state IN ('SUCCEEDED', 'FAILED', 'CANCELLED', 'UNCERTAIN', 'EXPIRED') THEN
      RETURN QUERY SELECT 'TERMINAL'::text, NULL::uuid, NULL::boolean;
      RETURN;
    END IF;
    IF incoming.state <> 'DISPATCH_PENDING'
       OR incoming.conversation_state <> 'BUSY' THEN
      RETURN QUERY SELECT 'UNAVAILABLE'::text, NULL::uuid, NULL::boolean;
      RETURN;
    END IF;

    -- Only a fresh admission needs incoming Lease/command ownership. Existing source/phase
    -- conflicts were classified first so a cross-Consumer reuse cannot be hidden by the global
    -- prepare-command primary key.
    SELECT lease.id,
           lease.deployment_id,
           lease.creator_id,
           lease.worker_id,
    ...
    SELECT command.command_id,
     ...
     WHERE command.command_id = input_prepare_command_id
     ...
     FOR UPDATE;
    IF NOT FOUND
       OR incoming_command.command_type IS DISTINCT FROM 'invocation.prepare'
       OR incoming_command.predecessor_command_id IS NOT NULL
       OR incoming_command.execution_capability_id IS DISTINCT FROM
            incoming.execution_capability_id
    ...
    IF incoming.agent_version_digest IS DISTINCT FROM input_agent_version_digest
       ...
       OR incoming.request_digest IS DISTINCT FROM input_request_digest
       ...
       OR incoming.execution_capability_digest IS DISTINCT FROM
            input_execution_capability_digest
    ...
    event_occurred_at := date_trunc('milliseconds', clock_timestamp());
    BEGIN
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'locked prepared Invocation changed unexpectedly'
          USING ERRCODE = '55000';
      END IF;
      ...
      IF incoming_command.state = 'SENT' THEN
        UPDATE public.broker_outbox AS command
           SET state = 'ACKED', acked_at = event_occurred_at
         WHERE command.command_id = input_prepare_command_id
           AND command.creator_id = input_creator_id
           AND command.state = 'SENT';
        IF NOT FOUND THEN
          RAISE EXCEPTION 'locked prepare command changed unexpectedly'
            USING ERRCODE = '55000';
        END IF;
      END IF;
    ...
    IF race_retried THEN
      CONTINUE classify_or_admit;
    END IF;
    RETURN QUERY SELECT 'ADMITTED'::text, NULL::uuid, NULL::boolean;
    RETURN;
  END LOOP classify_or_admit;
END;
$project_prepared$ LANGUAGE plpgsql;

REVOKE ALL ON FUNCTION public.creator_agent_project_prepared_fact_v1(
  uuid, uuid, text, integer, text, uuid, uuid, text, text, text,
  uuid, bigint, text, uuid, text
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.creator_agent_project_prepared_fact_v1(
  uuid, uuid, text, integer, text, uuid, uuid, text, text, text,
  uuid, bigint, text, uuid, text
) TO combo_agent_broker;

DO $prepared_admission_owner_gate$
DECLARE
...
BEGIN
  ...
  IF trusted_owner IS DISTINCT FROM true THEN
    RAISE EXCEPTION 'Prepared fact admission requires a trusted SECURITY DEFINER owner'
      USING ERRCODE = '42501';
  END IF;
END;
$prepared_admission_owner_gate$;

-- Preserve 0020's started/succeeded/failed rules. Only invocation.persisted is cut over: an old
-- Broker binary still has table INSERT for later lifecycle phases, but its direct prepared INSERT
-- now fails and rolls its preceding projection update back.
    ...
    IF NEW.event_type = 'invocation.persisted' THEN
      ...
      IF NOT COALESCE(privileged_session, false) THEN
        ...
        IF session_user <> 'combo_agent_broker'
           ...
           OR current_user IS DISTINCT FROM prepared_admission_owner THEN
          RAISE EXCEPTION 'invocation.persisted requires prepared fact admission authority'
            USING ERRCODE = '42501';
        END IF;
      END IF;
    END IF;
...
-- The existing Reconciler alert recorder and alert relation ACLs deliberately remain untouched.
