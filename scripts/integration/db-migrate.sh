...
for tbl in users tasks uploads capabilities sessions messages turns artifacts audit_llm_calls \
  ...
  agent_projects agent_revisions agent_tests agent_releases \
...
expected_tables='agent_projects,agent_releases,agent_revisions,agent_tests,artifacts,audit_llm_calls,auth_audit_events,auth_identities,auth_otp_challenges,auth_sessions,billing_accounts,billing_free_allowances,capabilities,messages,oauth_access_tokens,oauth_authorization_codes,oauth_authorization_requests,oauth_clients,oauth_refresh_tokens,payment_attempts,payment_callback_events,recharge_orders,sessions,tasks,turns,uploads,usage_charges,users,wallet_ledger'
