...
for tbl in users tasks uploads capabilities sessions messages turns artifacts audit_llm_calls \
  ...
  agent_projects agent_revisions agent_tests agent_releases; do
  exists="$(psql "$DATABASE_URL" -tAc "SELECT to_regclass('public.${tbl}') IS NOT NULL")"
  [ "$exists" = "t" ] || fail "缺基表 ${tbl}"
done
expected_tables='agent_projects,agent_releases,agent_revisions,agent_tests,artifacts,audit_llm_calls,auth_audit_events,auth_identities,auth_otp_challenges,auth_sessions,billing_accounts,billing_free_allowances,capabilities,messages,payment_attempts,payment_callback_events,recharge_orders,sessions,tasks,turns,uploads,usage_charges,users,wallet_ledger'
