#!/usr/bin/env bash
# VNext E4 Real Runtime — preflight（机制骨架，fail-closed）。
...
if [ ! -f "$ROOT_DIR/packages/creator-agent-protocol/dist/index.js" ] \
    && [ ! -d "$ROOT_DIR/packages/creator-agent-protocol/dist" ]; then
...
log '== E4 preflight: READY（真机执行步骤见 docs/测试/vnext-physical-gates-runbook-20260818.md §2）=='
