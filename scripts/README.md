...
`PR CI` 由 `pull_request` 触发，只运行格式、lint、类型、快速应用单测、关键 workflow 契约测试和 ShellCheck；它跳过 container contracts 与耗时的发布状态机模拟，不使用 Docker，不构建镜像或 release artifact。真实 Test 是独立的 `workflow_dispatch`：具有仓库写入权限的成员从 `main` 上受信任的控制器选择一个同仓库且仍开放的 PR，并提交其当时的精确 head SHA。控制器在启动时复验 PR 状态、仓库归属、分支 tip 和 SHA 可达性，冻结 controller 与候选身份，再调用该 controller 定义的可复用 Main CD 构建 attempt-scoped 的不可变 `combo-release-<SHA>-<source-CI-attempt>` artifact，随后仍使用同一 controller 固定的部署、校验和浏览器验收程序。PR 的 base 和祖先关系不参与 Test 授权；构建期间 `main` 或 PR 分支前进都不会改变已冻结快照，PR head 变化只会告警。后续步骤继续复验 PR 开放状态、同仓库归属、workflow run、run attempt、artifact ID 和 digest。`combo-dev` Environment 只允许 `main` 是为了阻止分支修改过的 workflow 读取 SSH 和 Resend Secret，不是对候选源码分支的限制。
