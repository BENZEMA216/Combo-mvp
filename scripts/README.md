...
- `scripts/integration/db-migrate.sh`：正式源码迁移头 `0020` 的空库、幂等、历史升级和角色验证；最后构建 authoring 编译器依赖并运行私有 Draft HTTP/PG 测试（对象存储假件）。只用于临时测试库，不能对常驻 Test/Preview/Production 执行；私有 Draft 测试另有本地连接与测试库名保护。
