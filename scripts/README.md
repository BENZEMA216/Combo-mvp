...
- `test-truth.mjs` 与 `tests/test-suites/manifest.v1.json`：校验分层测试目录、从 Vitest JSON 与 Node test 事件流逐 case 推导 PASS/FAIL，并生成绑定候选 SHA 的 PR truth report。CI 上传脱敏后的框架报告并在聚合时复核 SHA-256；缺报告、零测试、cancel、skip 或 todo 都不能变成 PASS。所有未登记测试明确属于 `NO_EVIDENCE_CLAIMS`；基线后新增的未登记文件会单独计数，但不能产生证据声明，后续只能通过独立治理 PR 将已存在的文件升级为 evidence suite。Playwright 原始 JSON 只能在受限临时目录由独立 normalizer 读取；normalizer 会拒绝零测试、命令行过滤或命令行浏览器覆盖、`.only`、预期失败、skip、retry/flaky、全局错误、候选或运行策略漂移及可能泄露内容的 attachment/stdout/stderr，公开机器报告只保留合成用例编号、相对路径、状态与计数；调用方必须提供由治理策略锁定的最小用例数，该 ratchet 只防止用例数量收缩，不代表锁定历史用例身份或断言语义。CLI 的 Chromium 默认值不证明逐用例实际浏览器，product producer 必须在真实用例内部断言 effective `browserName`，后续 governance activation 才能据此授予有限 Browser claim。仅有 normalizer 不代表浏览器 producer 已接线，现有浏览器 suite 在独立 product producer 与后续 governance activation 完成前仍保持 `NOT_IMPLEMENTED`；真实 Codex 与 Desktop UAT 也不会由普通 PR CI 合成 PASS。
