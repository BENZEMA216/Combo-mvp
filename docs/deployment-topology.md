...
- Billing 的 `BILLING_PAYMENTS_ENABLED` 默认关闭。开启时须配置独立支付凭证密钥、Gateway 准入凭据、受信 Authz 地址与 Host 来源；当前 Host 接口只实现 Cookie 认证，尚未提供支付专用 Bearer。收银台页面与渠道适配仍缺失，不能只打开开关就对外提供支付。本次源码变更不改变 V2 入口、路由或现有运行服务。
