...
- 支付 SDK 已按锁定的 Combo 协议实现并完成合同测试。Billing 的 `/v1/payments` 路由与 PostgreSQL 支付基础位于独立模块，尚未接入进程入口、用户认证或真实渠道，未部署。
- 标准 402 的服务端生成与支付资金预留尚未实现。
- 每 Agent 短期身份与 Gateway 用户身份重验尚未实现。
