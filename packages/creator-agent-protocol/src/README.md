...
- `agent.ts`：显式 `./agent` 子路径的 Agent Definition、按 revision 修订的不可变 DraftSnapshot 与不可变 AgentVersion；固定 Git Project 快照、只读本机执行 profile 和 domain-separated fingerprint。
