...
`agent-package-project-history-contract.test.ts` 锁定 Project-history Draft V3 的低保证来源字段、计数上限、
credential 词法拒绝、Draft fingerprint 到 Package digest 的 provenance 绑定、Share V2 全文件清单验证、
cleartext runtime projection 和普通用户可读 launch prompt 的 exact grammar/8 KiB 上限。它还拒绝 tamper、额外/
缺失/重排文件、nested Proxy/getter/cycle/depth/giant-key 以及把 JSON、runtime 或内部 ID 混入用户可见首轮 prompt。
