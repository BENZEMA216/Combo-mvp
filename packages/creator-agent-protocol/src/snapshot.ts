...
const FORBIDDEN_SEGMENTS = new Set(['.git', 'node_modules', '.ssh', '.codex']);
...
export const SnapshotPathSchema = z
  ...
  .refine((value) => !/[\u0000-\u001f\u007f]/u.test(value), '拒绝控制字符')
  .refine(
    (value) =>
      value
        .split('/')
  ...
  .refine((value) => {
    ...
    return !segments.some(
      (segment) =>
        FORBIDDEN_SEGMENTS.has(segment) || segment === '.env' || segment.startsWith('.env.'),
    );
  ...
  .object({
    ...
    pathPolicy: z
      .object({
        ...
        collision: z.literal('nfc-plus-ecmascript-simple-case-fold'),
      })
      .strict(),
    archive: z
      .object({
        format: z.literal('pax'),
        tarImplementation: z.literal('tar-stream@3.2.0'),
        zstdImplementation: z.literal('zstd@1.5.7'),
