...
Project-history Package 还包含固定路径 `skills/extracted-method/starter-prompts.json`。该文件使用
`combo.agent-package-starter-prompts/1` 的规范 JSON，并和其他文件一起进入 manifest 与 Package digest；
Share parser 必须把公开顶层 `starterPrompts` 与这里的顺序和文本逐项核对。因此即使数据库中的 Share JSON
摘要被同步重算，也不能在不改变 Package digest 的情况下替换 prepare/launch 使用的起始任务。
...
同一子路径的 V3 request/Draft 专门表达用户在 Host 中显式选择 saved Project 后，由 Project task history
提炼出的 typed candidate。V3 只接受固定枚举与有界计数，不接受 Project/task/thread/session/item ID、路径、
消息、transcript 或 raw tool output；caller 不计算历史 digest，server 只对 exact submitted candidate、制作要求
与计数生成 consistency commitment。`assurance=best_effort`，coverage、Host attestation 与进入模型前的字段
projection 都固定 `not_proven`。服务端会拒绝常见 assignment、Bearer、private-key 和中英文 marker 形式的
明显凭据值，但该词法 guard 不是完整脱敏或无原文证明。`discoveredThreadCount` 表示按稳定顺序
已选的 eligible Codex 任务（最多 20），必须与逐一读完的 `readThreadCount` 相等；其余同 Project
matching/non-Codex 任务全部计入 `omittedThreadCount`（最多 10,000）。Host 的 pinned 结果可以使
`list_threads(limit:50)` 返回超过 50 条，因此不声明总数上限为 50。V1/V2/V3 fingerprint domain 彼此独立。

显式 `agent-package-share` 子路径定义 immutable `combo.agent-package-share/2` 与
`COMBO_AGENT_PACKAGE_RUN/2`。Share 携带 exact Package canonical bytes，并逐项核对 inventory 的 path、order、
byteLength、file digest 与 package digest，同时把顶层 starter 与 Package 内 digest-bound starter manifest
逐项绑定；Package provenance 绑定 V3 source Draft fingerprint。Run V2 返回
服务端验证的 cleartext `AGENT.md`/`SKILL.md` runtime projection，避免模型自行解 base64，并限制为 64 KiB。
`executionBoundary.hostInstalledEnforcement=not_proven`：投影完整性不等于 Host 已安装或技术强制 Agent。同一子路径还定义
服务端生成的普通用户可读 receiver launch prompt：它只显示 Agent 名、公开 share URL、唯一 Package
digest 与 exact starter，不显示 JSON、runtime material、Draft/Release/token 或内部协议；规范文法最大 4096
字符且 UTF-8 不超过 8 KiB。`COMBO_AGENT_PACKAGE_RUN/2` 可留作机器证据，不能用作 Project B
的首条用户可见消息。
...
- `@cb/creator-agent-protocol/agent-package-draft`：暴露一句制作要求、可修订 Package Draft、乐观 revision
  请求、Creator bootstrap handoff、Project V1、current-conversation V2 与 Project-history V3 的 domain-separated draft
  fingerprint 和规范化解析；它不暴露 Host snapshot 或任务选择 API，不保存绝对 Project 路径或 raw
  transcript，也不执行提取、编译、发布或推理。
- `@cb/creator-agent-protocol/agent-package-share`：暴露 exact Package byte bundle、immutable Share V2、
  cleartext runtime projection、Run V2 和普通用户可读 launch prompt 的严格生成/解析；它不实现 registry、
  OAuth、Host Project 选择或 Host-installed enforcement。
