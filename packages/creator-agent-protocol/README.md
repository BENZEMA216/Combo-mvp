...
- `src/` 定义共享协议（包括 Snapshot Manifest、archive/manifest 两类加密 Envelope、AgentVersion、Broker、Worker Conversation Ready/Invocation durable facts、Sandbox 与 HTTP）、RFC 8785 JSON 规范化、Invocation 状态机、稳定错误和重试策略、Evidence Bundle、不变量、测试注册表，以及 AgentVersion、Execution Capability 上游计数、Snapshot Manifest 总量、单文件、压缩对象、压缩比和相对路径 UTF-8 bytes 的独立边界证据目录。
- `fixtures/` 保存 Schema、WSS、状态机、canonical digest、AgentVersion、Execution Capability 上游计数、Snapshot Manifest 总量、单文件、压缩对象数值、压缩比与相对路径 UTF-8 bytes 资源边界和 Evidence Bundle 的 golden vectors；`index.json` 绑定每个 fixture 的 SHA-256。
