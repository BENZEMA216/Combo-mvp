...
显式 `agent` 子路径还提供 strict `combo.creator-agent-draft-handoff/1`。handoff 只包装一个完整、已
fingerprint 的 DraftSnapshot，要求 `authoringSource=codex_current_task` 且 `rawStored=false`，并只接受 exact
canonical JSON；这些字段仍是内容声明，不是来源认证或自动脱敏证明。
...
- `@cb/creator-agent-protocol/agent`：暴露严格 Definition、Draft、current-task Draft handoff、Version、
  freeze/verify 和 canonical Draft/Version 序列化；它不证明作者身份、实际脱敏、用户确认、Git remote
  可达或 OS 级 Project 隔离。
