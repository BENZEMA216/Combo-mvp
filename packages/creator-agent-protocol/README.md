...
- `src/` 定义共享协议（包括 Snapshot Manifest、archive/manifest 两类加密 Envelope、AgentVersion、Broker、Worker Conversation Ready/Invocation durable facts、Sandbox 与 HTTP）、RFC 8785 JSON 规范化、Invocation 状态机、稳定错误和重试策略、Evidence Bundle、不变量、测试注册表，以及 AgentVersion、闭世界 Context Tools、Execution Capability 上游计数、Snapshot Manifest 总量、单文件、压缩对象、压缩比和相对路径 UTF-8 bytes 的独立边界证据目录。
- `fixtures/` 保存 Schema、WSS、状态机、canonical digest、AgentVersion、闭世界 Context Tools、Execution Capability 上游计数、Snapshot Manifest 总量、单文件、压缩对象数值、压缩比与相对路径 UTF-8 bytes 资源边界和 Evidence Bundle 的 golden vectors；`index.json` 绑定每个 fixture 的 SHA-256。
...
闭世界 Context Tools corpus 直接引用技术方案 7.8 与 ADR-VNEXT-013/031，以 exact、empty、missing、reordered、duplicate、unknown replacement 和 unknown extra 七个 digest-bound 变体覆盖四个运行时 parser、四个公开 contract/OpenAPI owner 和本地 `buildAgentVersion` 共 63 个结果；它只证明三工具 tuple 不能删减、重排、重复或扩张，不证明 G2 isolated runtime、E4 real Linux Codex 或 E5 real Mac isolation，因此 `SCH-004`、`SCH-009`、`SCH-010` 均保持 `planned`。

Broker whole-wire corpus 把 runtime 的 65,536-byte frame ceiling 与当前 `broker-contract.v1.json#/maxFrameBytes` 对齐：两个 Protocol parser 执行 6 个 N-1/N/N+1 结果，真实回环 Gateway handshake/established 与 Worker first-lease/established 再执行 12 个结果。N+1 target payload 不进入 Gateway authenticate/accept/replay/gap 或 Worker activate/commitInbound/replay/gap；transport 关闭仍可触发正常的 `closeSession`、`releaseConnection`、重连和新 Lease lifecycle。该值只作为 runtime-to-advertised alignment，不由本测试冻结成产品 policy；证据不覆盖 TLS/public WSS、fragmentation/extension negotiation、load/backpressure、E3 公网云或 Production，且不完成 `SCH-004`。

Raw-ingress hostile corpus 对 12 个直接公开 JSON ingress owner 执行同一套 76 probes：Broker handshake/frame、Snapshot preparation/commit marker、Evidence 七个 structured byte owner 与 Snapshot Manifest，共 912 个结果。五类 malformed UTF-8、escaped lone surrogate、除 TAB/LF/CR 外的 62 个 C0/C1 与 BOM/syntax/duplicate/unknown 输入都必须得到稳定低基数错误且不回显 canary；每次调用只证明输入 bytes 未被修改，不把纯 parser 性质冒充 Gateway/Worker/S3/PG business zero-mutation。该 evidence 不覆盖全部 HTTP request roots、generic JSON/YAML error surface、Content-Encoding/chunked、T0 Linux 或 `SNP-010` 的真实 path Gate，`SCH-005` 与 `SNP-010` 继续保持 `planned`。

资源边界 corpus 只证明运行时 parser 与公开 contract/OpenAPI 在已声明数值上的一致性，不替代产品政策冻结。Broker capacity corpus 以 0/1/2 冻结握手中的两个 singleton 容量字段，并覆盖 Protocol、真实 Gateway 回环 WebSocket/WS 与真实 Worker 出站握手；它不证明 TLS/WSS termination、public ingress、Creator/Conversation WIP、queue、PostgreSQL/SQLite admission、真实推理容量或 soak。Execution Capability 上游计数 corpus 直接引用 P0 `INV-010`、ADR-VNEXT-010 和测试方案的 at-most-once/Provider sink 断言，以同一组 `UNUSED=0`、`DISPATCHED=1`、非法 `DISPATCHED=2` 记录执行 Schema、决策函数、公开 contract 和真实文件 SQLite 共 12 个结果；它只证明数值边界与 SQLite `CHECK`/重启读取，不证明真实 Provider sink、并发抢占、crash/network fault、10k 随机序列、E4 或 E6，也不完成 `INV-010`。Snapshot Manifest 的文件总数 2,000 与展开后 200 MiB 直接引用技术方案 5.2 和测试方案 SNP-002/003/006/007；独立的单文件 corpus 引用同一技术方案、SNP-004/005 和既有 ADR-VNEXT-003，并由 Snapshot 包使用真实非稀疏 UTF-8 文件补充 build-to-verify 机制证据。压缩对象 corpus 引用技术方案 5.2、SNP-008、ADR-VNEXT-003 与 ADR-VNEXT-011，只冻结 50 MiB 明文 metadata、36-byte AEAD framing 和公开数值 owner 的一致性，不构造 50 MiB archive，也不证明真实 tar/zstd 机制 Gate。压缩比 corpus 引用技术方案 5.2、测试方案 8.3 与 ADR-VNEXT-003，以 digest-bound 99/100/101 整数 probes 对齐决策与运行时 helper；Snapshot 包另行生成 1 MiB counter-hex accepted archive 和 1 MiB ASCII `a` canonical bomb，并在 fake S3 发布流程证明 bomb 不产生 preparation、final 或 commit。该本机 deterministic 证据不把 accepted 向量冒充 exact 100:1、不观测 decompressor 调用次数，也不替代 SNP-008、T0 Linux、E2 MinIO/PostgreSQL 或云端状态。相对路径 corpus 引用技术方案 5.2、SCH-004、SNP-009 和 ADR-VNEXT-004，以 ASCII/CJK/astral 混合路径冻结 512 UTF-8 bytes 完整路径 owner，并由 Snapshot 包补充真实文件系统 build-to-verify；文件系统 component 255-byte 政策尚未冻结。`Utf8TextSchema` 的 19 个 runtime source owner 与 47 个 contract/Broker/OpenAPI public node 已通过同一 portable scalar-control pattern 收口，并分别使用 ASCII、CJK 和 emoji 在 N-1/N/N+1 精确 UTF-8 byte 边界执行 594 个实际 owner 结果；所有超限错误均命中目标 byte issue 且不回显 canary。Snapshot path 继续使用独立且更严格的 pattern 与 owner 边界。Signed PUT URL 的 2 个 runtime target owner、4 个 response 语义 owner与 3 个物理公开节点也已在 WHATWG URL 解析前使用同一严格 scalar/no-control pattern，阻止控制字符或 surrogate 被静默规范化；该证据不新增 URL maxLength，也不扩张到一般 URL canonicalization。`UnicodeCodePointStringSchema` 的 29 个 runtime owner 与 47 个 contract/OpenAPI public node 已用同一 portable scalar-control pattern 收口，并以 68 个 compact probes 在 8 组 code-point helper 边界执行 5,700 个结果；另用固定 canary 加 astral 字符在 29 个 runtime owner 与 47 个公开节点执行 N-1/N/N+1 共 228 个精确 code-point maximum 结果，并保持两个 nullable owner 的 runtime/contract `null` 语义。其中 26 个普通 owner 接受 N-1/N 且在 N+1 命中目标长度 issue；3 个 exact-derived object key owner 的真实基线保持有效，任意 N-1/N probe 仅由精确派生约束拒绝，N+1 同时命中派生与长度约束；全部 35 个 runtime 拒绝和 47 个公开 N+1 错误都不回显 canary。Compatibility、decoded/UTF-8/structural/resource corpus 的 22 个实际 plain metadata owner 已复用 required/optional strict scalar/no-control schema，保留合法 CJK/astral 与空 root pointer，同时拒绝 lone surrogate、NUL 和 C1；duplicate-JSON-key 错误也只保留稳定原因与 offset，不再回显原 key。Broker handshake/frame 与 Snapshot preparation/commit marker 四个公开 raw parser 已统一抛出只含固定 `name/message/code` 的 `ProtocolRawInputError`，不再暴露 Zod issues、unknown key、原始输入或 cause；generic Zod schema 的直接调用、完整 raw-byte ingress 证据与 Idempotency alias ADR 仍未完成，因此 `SCH-005` 与 `SNP-010` 保持 `planned`。AgentVersion `developerInstructions=32` 仍需资源上限 ADR。`SCH-004`、`SNP-008` 与 `SNP-009` 保持 `planned`。
