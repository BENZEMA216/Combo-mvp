...
默认 `test` 不隐式要求 Docker，并排除 `.integration.test.ts`；显式 `test:minio:e2` 产生真实 MinIO adapter 的 E2 组件证据。它仍不测试 PostgreSQL、正式 IAM、encrypted manifest 对象、真实 RFC3394 wrap/KEK/KMS、Retention、备份或 DR，也不能单独宣称 Gate 1 完整通过。
