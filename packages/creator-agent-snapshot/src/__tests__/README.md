...
- Snapshot 测试检查创建顺序、mtime、mode、单 byte 变化、长路径、文件数和大小边界；独立资源边界测试把同一份 digest-bound Manifest fixture 的文件数与展开字节 N-1/N/N+1 送入生产 `parseSnapshotManifest`，并把真实非稀疏 UTF-8 单文件的 10 MiB N-1/N/N+1 送入 `buildSnapshotFromProject`，对两个合法边界完整执行 `verifySnapshotArchive`。压缩对象边界测试把 digest-bound metadata 的 50 MiB N-1/N/N+1 送入四个生产数值 owner，并证明 N+1 在 S3 client 与 presigner 调用前拒绝；它不构造真实 50 MiB tar/zstd archive，`SNP-008` 仍为 `planned`。压缩比测试用 99/100/101 数值 probes 锁定整数比较，再现场生成 1 MiB counter-hex accepted archive 与 1 MiB ASCII `a` canonical bomb；前者完整 verify，后者得到稳定 ratio error，fake S3 finalize 不产生 preparation、final、commit 或 reader visibility，并清零 unwrap key。测试不会提交大对象，也不声称 accepted archive 恰为 100:1 或已经观测 decompressor 调用次数。相对路径边界测试使用真实 ASCII/CJK/astral 混合目录，把 511/512/513 UTF-8 bytes 三个 digest-bound probe 送入 `buildSnapshotFromProject`；前两个完整执行 `verifySnapshotArchive`，513 bytes 以 `SNAPSHOT_PATH_TOO_LONG` 在 verifier 前拒绝。该本机证据不冒充 T0 Linux/E1，component 255-byte 政策仍未冻结，`SNP-009` 保持 `planned`。
