...
- `trial.ts` 定义试用域：会话、消息、产物和运行中 Turn 的视图，以及建会话、发消息的请求体；Artifact 会带可选来源 Turn 和创建时间，会话详情能从 PostgreSQL 恢复 active Turn。会话详情里的能力摘要带开场表单字段与提示语（来自能力定义，定义读不出时为空数组）。消息内容是 agent 原生分块格式，共享层只约束到「是数组」，严格校验在 runtime 侧。
