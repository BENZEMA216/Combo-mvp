...
- `domains/` 按业务域定义数据传输对象与校验规则。认证域包含邮箱 challenge、邮箱 verification、当前用户、登出、成功包络、显式安全 Cookie 策略和站内回跳净化函数；其余文件覆盖任务、能力项、Agent 项目与不可变版本、不可变 Git Project 分享、远程 MCP OAuth、试用会话、Artifact 来源、active Turn 恢复信息与去敏规则。
