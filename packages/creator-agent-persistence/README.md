...
本包是本地、尚未发布的 Creator Agent Catalog。它用独立 Node 24 SQLite 保存用户已经看见的 V1 或 V2
Draft handoff、按 revision 修订的不可执行 DraftSnapshot，以及经确认冻结的 immutable AgentVersion。V1
继续兼容 `combo.creator-agent-draft-handoff/1`；Project Context Compiler 产出的 V2 使用
`combo.creator-agent-draft-handoff/2`，并在 Draft 内持久化 compact Project source ledger。本包不复用 Worker
...
- 导入只接受 protocol parser 验证后的 exact canonical UTF-8 JSON，并按明确 protocol 分派 V1 或 V2。同一
  handoff exact replay 返回原 Draft；相同 Draft identity 配不同内容、revision gap 或 base Version 漂移都会
  fail-closed。两代对象沿用现有 JSON row 与 SQLite schema，可以在同一个 Catalog 中冻结并重开，不需要
  schema migration。
...
- Catalog 保存经过人工审阅的 Definition、Draft、handoff wrapper 与 Version 内容。V2 Draft 中只包含
