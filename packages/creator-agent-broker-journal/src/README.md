...
- `host-projection.ts`：解包 committed start/interrupt/observe wrapper，在同一 scope 内调用并验证 exact R1 handle；outcome observer 在内部完成 verify + seal，最终只返回 terminal projection。
...
- `sqlite-store-internal.ts`：package-internal durable limit 与仅测试可见的 clock/fault hook symbol，不从 package exports 暴露。
- `sqlite-store.ts`：独占连接、owner epoch、CAS cursor、exact replay、32 KiB seal、原子 outbox，以及本地 transport `ENQUEUED` handoff marker。
...
state/outbox/seal atomicity 已由 store 负责；handoff marker 不代表 Broker/Cloud ACK，delivery ACK 和
retention 留给后续独立切片。
