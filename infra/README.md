...
- `Dockerfile.api`、`Dockerfile.runtime` 和 `Dockerfile.web` 构建三个生产镜像。authoring 与 runtime 镜像都携带 Creator Agent Protocol 与 Shared 的运行时产物；authoring 镜像还包含迁移入口所需的应用角色配置脚本。`Dockerfile.resend-mock` 只构建测试替身。
