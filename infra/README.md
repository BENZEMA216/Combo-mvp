...
- `Dockerfile.api`、`Dockerfile.agent-gateway`、`Dockerfile.runtime` 和 `Dockerfile.web` 构建四个 release 镜像。Agent Gateway 镜像目前只会由 schema v2 部署到 Test；authoring 镜像同时包含迁移入口所需的应用角色配置脚本。`Dockerfile.resend-mock` 只构建测试替身。
