...
- `Dockerfile.api`、`Dockerfile.runtime` 和 `Dockerfile.web` 构建三个生产镜像。authoring 镜像同时包含迁移入口所需的应用角色配置脚本。`Dockerfile.resend-mock` 只构建测试替身。`Dockerfile.v2` 与 `entrypoint-v2.sh` 构建 combo-v2 验证栈的单镜像（authz / billing / llm-gateway 按 PROCESS 分叉，迁移 Job 覆盖 command），只用于手工验证部署。
