...
- `Dockerfile.api`、`Dockerfile.runtime` 和 `Dockerfile.web` 构建三个生产镜像。authoring 镜像携带 Creator Agent Protocol 与 Shared 的运行时产物，并同时包含迁移入口所需的应用角色配置脚本。`Dockerfile.resend-mock` 只构建测试替身。
