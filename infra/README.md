...
- `nginx.conf` 把 React、authoring API 和 runtime API 放在同一个站点下，使两端共享主机限定的 HttpOnly 会话 Cookie，并为 Project Agent 与当前任务派生 Codex Agent 两类公开分享页添加禁止索引、禁止来源泄露和禁止缓存响应头。生产 Cookie 使用 `__Host-` 前缀、Secure、根路径且没有 Domain；本地 HTTP 测试使用无前缀名称。访问日志只保留请求方法、响应状态和耗时；请求期错误日志写入空设备，避免上游故障把客户端地址、Cookie、原始 URL 或查询字符串写入容器日志。
