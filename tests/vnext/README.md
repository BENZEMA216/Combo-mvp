...
`pnpm -F @cb/creator-agent-protocol generate:contracts` 会在重建三份公开契约后，级联刷新 Protocol 登记的 digest-bound corpus、fixture index 和本目录 case registry。级联只重算指向仓库内实际文件的 bytes digest 与明确登记的 artifact digest，不改写 probe、canonical input 或其他语义 digest；命令连续运行必须保持无新增差异。
